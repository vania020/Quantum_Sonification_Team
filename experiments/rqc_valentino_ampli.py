...
# --- 3. EXTRAER MAGNITUDES Y FASES DE UN CIRCUITO ---
def get_magnitudes_phases_from_circuit(qc, use_mps=False):
    qc_temp = qc.copy()
...
def generate_quantum_audio_data_by_layer(num_qubits, layers, use_mps=False):
    ...
    magnitudes, phases = get_magnitudes_phases_from_circuit(qc, use_mps)
    all_layers_data.append((0, magnitudes, phases))
    ...
    for layer_idx in range(layers):
        ...
        magnitudes, phases = get_magnitudes_phases_from_circuit(qc, use_mps)
        all_layers_data.append((layer_idx + 1, magnitudes, phases))
...
if __name__ == "__main__":
    ...
    for layer_number, mags, phas in layer_data:
        ...
        export_to_wav(mags, phas, filename)
